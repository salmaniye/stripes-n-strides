...
import { register, login, getAchievement, postAchievement } from './utils/apiEndpoints';
...
describe('postAchievement function', () => {
    it('should return error message if not all needed data is provided', async () => {
        const result = await postAchievement('', 1, 10, 100); // Not providing user_id
        expect(result.status).toBe(false);
    ...
    it('should return success message if POST request is successful', async () => {
        const result = await postAchievement(12, 15, 10, 1000); // Providing all needed data
        expect(result.status).toBe(true);
